...
export default class ExpressionDecorator extends QueryDecorator {
  ...
  async build(): Promise<QueryContext> {
    ...
    context.expressionSelect.push(
      ...this.parsedExpressions.map((e) => e.baseExpressionClause),
    );
    context.conditions ??= {};
  ...
  public setWhereClauses(): void {
    this.whereClauses = this.parsedExpressions.flatMap((expr) =>
  ...
  public setHavingClauses(): void {
    this.havingClauses = this.parsedExpressions.flatMap((expr) =>
      expr.havingClause ? expr.havingClause : [],
    );
  }
