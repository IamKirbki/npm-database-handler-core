...
export class SqlGenerator {
    ...
    private renderWhere(): string {
        if (
            ...
            throw new InvalidWhereConditionError(
                'Date objects are not supported in WHERE conditions.',
            );
        }
    ...
    private buildWhereWithOperators(
        where: (QueryComparisonParameters & { fromOperator?: boolean })[],
    ): string {
        return where
            .map((condition) => {
