...
export class QueryStatementBuilder {
  ...
  constructor(
    ...
    tableColumnInformation?: Map<string, TableColumnInfo[]>,
  ) {
  ...
  private async buildBaseLayer(): Promise<string> {
    if (!this._layers.base.from) {
      throw new InvalidOperationError(
        "Base layer must specify 'from' table name.",
      );
    }

    let builder: IQueryBuilder = new BaseSelectQueryBuilder(
      this._layers.base.from,
      this._layers.base.select || [],
      this._layers.base.joinsSelect || [],
      this._layers.base.expressionsSelect || [],
    );

    if (this._layers.base.joins && this._layers.base.joins.length > 0) {
      builder = new JoinDecorator(
        builder,
        {
    ...
    if (
      ...
      if (builder instanceof ExpressionDecorator) {
        ...
        this._layers.pretty.where = this.addUnique(
          this._layers.pretty.where,
          builder.whereClauses,
        );
        this._layers.pretty.groupBy = this.addUnique(
          this._layers.pretty.groupBy,
          builder.groupByClauses,
        );
        this._layers.pretty.having = this.addUnique(
          this._layers.pretty.having,
          builder.havingClauses,
        );
        this._layers.base.orderBy = this.addUnique(
          this._layers.base.orderBy?.map((ob) => ({
            column: `${ob.column}`,
      ...
      if (this._layers.base.where) {
        builder = new WhereDecorator(
          builder,
          this._layers.base.joins
            ? QueryStatementBuilder.normalizeAndQualifyConditions(
  ...
  private async buildPrettyLayer(sql: string): Promise<string> {
    let builder: IQueryBuilder = new BaseSelectQueryBuilder(
      `( ${sql} ) AS BASE_QUERY`,
      [
        ...(this._contexts.base?.select || []),
        ...(this._layers.pretty?.select || []),
      ],
      this._contexts.base?.joinsSelect?.map((j) => j.split('AS')[1].trim()) ||
      [],
    );
    ...
    if (this._layers.pretty) {
      const expressions = this._layers.pretty.expressions?.length
        ? QueryExpressionBuilder.buildExpressionsPart(
          this._layers.pretty.expressions,
        )
        : [];
      ...
      if (expressions.length > 0) {
        ...
        if (builder instanceof ExpressionDecorator) {
          ...
          this._layers.final ??= {};
          this._layers.final.orderBy = this.addUnique(
            this._layers.final.orderBy?.map((ob) => ({
  ...
  private async buildFinalLayer(sql: string): Promise<string> {
    ...
    if (this._layers.final) {
      ...
      if (this._layers.final.limit) {
        builder = new LimitDecorator(
          builder,
          this._layers.final.limit,
          this._layers.final.offset,
        );
      }
    }
  ...
  public static normalizeAndQualifyConditions(
    ...
    const normalizedWhere: QueryComparisonParameters[] = where;
    ...
    return normalizedWhere
      ...
      .filter((cond) => cond !== undefined) as QueryComparisonParameters[];
  }
