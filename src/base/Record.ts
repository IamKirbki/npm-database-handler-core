...
export default class Record<ColumnValuesType extends columnType> {
  ...
  public async Insert(): Promise<this | undefined> {
    ...
    const queryStr = await DepricatedQueryStatementBuilder.BuildInsert(
      this._tableName,
      this._values,
    );
    ...
    if (Array.isArray(this._values)) {
      recordId =
        ...
        this._values.map((v) => (v.column === 'id' ? v.value : undefined));
    } else {
    ...
    const whereParams: QueryComparisonParameters[] = Object.entries(
      this._values,
    ).map(([column, value]) => ({
      column,
      operator: '=',
  ...
  public async Update(
    ...
    whereParameters: QueryIsEqualParameter,
  ): Promise<this> {
    const originalValues = this._values as Partial<ColumnValuesType>;
    if (
      (originalValues as object & ModelWithTimestamps).updated_at !== undefined
    ) {
      (newValues as object & ModelWithTimestamps).updated_at =
    ...
    const queryStr = await DepricatedQueryStatementBuilder.BuildUpdate(
      this._tableName,
      newValues as QueryIsEqualParameter,
      whereParams,
    );
  ...
  public async Delete(primaryKey?: QueryIsEqualParameter): Promise<void> {
    const originalValues = this._values as Partial<ColumnValuesType>;
    if (
      (originalValues as object & ModelWithTimestamps).deleted_at !== undefined
    ) {
      (this._values as object & ModelWithTimestamps).deleted_at =
        new Date().toISOString();
      await this.Update(
        this._values,
        this._values.id ? { id: this._values.id } : primaryKey || {},
      );
      return;
    }
    ...
    const queryStr = await DepricatedQueryStatementBuilder.BuildDelete(
      this._tableName,
      whereParams,
    );
    const _query = this._queryFactory.create({
